...
export class Canvas extends EventCenter {
    ...
    __onMouseDown(e: MouseEvent) {
        ...
        let isLeftClick = 'which' in e ? e.which === 1 : (e as MouseEvent).button === 1;
    ...
    _isTargetTransparent(target: FabricObject, x: number, y: number, tolerance: number = 0) {
        ...
        let imageData = cacheContext.getImageData(x, y, tolerance * 2 || 1, tolerance * 2 || 1) as any;
    ...
    _chooseObjectsToRender() {
        ...
        // 当前有没有激活的组（也就是多个物体）
        let activeGroup: any = this.getActiveGroup();
        // 最终要渲染的物体顺序，也就是把激活的物体放在后面绘制
        let objsToRender: any = [];
